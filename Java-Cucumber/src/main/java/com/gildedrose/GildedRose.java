...
class GildedRose {
    ...
    public void updateQuality() {
        for (int i = 0; i < items.length; i++) {
            if (!items[i].name.equals(ITEM__AGED_BRIE)
                    && !items[i].name.equals(ITEM__BACKSTAGE_PASSES)) {
                if (items[i].quality > 0) {
                    if (!items[i].name.equals(ITEM__SULFURAS)) {
                ...
                if (items[i].quality < 50) {
                    ...
                    if (items[i].name.equals(ITEM__BACKSTAGE_PASSES)) {
